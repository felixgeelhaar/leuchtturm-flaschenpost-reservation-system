...
describe('Environment Configuration', () => {
  ...
  describe('Server Environment Validation', () => {
    it('validates complete server environment successfully', async () => {
      ...
      expect(serverConfig?.database.supabaseServiceRoleKey).toBe('test-service-role-key');
      expect(serverConfig?.auth.jwtSecret).toBe('this-is-a-32-character-jwt-secret');
      expect(serverConfig?.gdpr.privacyContactEmail).toBe('privacy@example.com');
    });
    ...
    it('validates server environment with optional fields', async () => {
      ...
      expect(serverConfig?.monitoring?.sentryDsn).toBe('https://sentry.example.com/123');
    });
  ...
  describe('Client Environment Validation', () => {
    ...
    it('uses defaults when client environment validation fails', async () => {
      ...
      expect(console.warn).toHaveBeenCalledWith(
        expect.stringContaining('⚠️ Client environment validation failed, using defaults:'),
        expect.any(Object),
      );
    });
  ...
  describe('Environment Detection', () => {
    it('returns null configs when neither server nor client environment is available', async () => {
      ...
      const { serverConfig, clientConfig } = await import('@/lib/config/environment');
