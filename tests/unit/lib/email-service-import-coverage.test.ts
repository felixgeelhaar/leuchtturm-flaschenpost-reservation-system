...
describe('Email Service - Import Coverage Tests', () => {
  ...
  describe('Fallback Email Service', () => {
    ...
    it('fallback methods throw meaningful errors', async () => {
      ...
      await expect(emailService.sendCancellationConfirmation({} as any))
        .rejects
        .toThrow('Email service not configured');
      
      await expect(emailService.sendPickupReminder({} as any))
        .rejects
        .toThrow('Email service not configured');
  ...
  describe('Template Generation Code Paths', () => {
    ...
    it('handles shipping method in templates', async () => {
      ...
      await expect(emailService.sendReservationConfirmation(shippingData)).resolves.not.toThrow();
    });
    ...
    it('handles picture orders in templates', async () => {
      ...
      await expect(emailService.sendReservationConfirmation(pictureOrderData)).resolves.not.toThrow();
    });
  });
  ...
  describe('Error Handling Code Paths', () => {
    it('handles email sending timeout', async () => {
      ...
      await expect(emailService.sendReservationConfirmation(mockData))
        .rejects
    ...
    it('handles verification failure', async () => {
      ...
      await expect(emailService.verifyConnection())
        .rejects
        .toThrow('Email service verification failed');
    });
  });
});