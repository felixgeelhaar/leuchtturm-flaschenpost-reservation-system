...
vi.mock('@/config/content', () => ({
  websiteContent: {
    kindergarten: { name: 'Test Kindergarten', address: 'Test St. 123, Berlin' },
...
describe('EmailService - Working Tests', () => {
  ...
  describe('Reservation Confirmation Emails', () => {
    it('sends pickup confirmation email successfully', async () => {
      ...
      expect(emailOptions.headers['X-Reservation-ID']).toBe(mockReservationPickup.id);
    });
    ...
    it('handles email send timeout', async () => {
      ...
      mockSendMail.mockImplementation(() => new Promise(resolve => setTimeout(resolve, 15000)));
  ...
  describe('Email Content Generation', () => {
    ...
    it('includes magazine and user details in content', async () => {
      ...
      expect(emailOptions.html).toContain(mockReservationPickup.quantity.toString());
    });
  });
