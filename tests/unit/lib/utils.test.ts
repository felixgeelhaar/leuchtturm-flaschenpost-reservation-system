...
describe('Utility Functions', () => {
  ...
  describe('Validation Utilities', () => {
    ...
    it('validates quantity constraints', () => {
      const validateQuantity = (quantity: number, maxAvailable: number): boolean => {
  ...
  describe('Array and Object Utilities', () => {
    ...
    it('filters and maps arrays correctly', () => {
      ...
      const activeMagazines = magazines.filter(m => m.isActive && m.availableCopies > 0);
      ...
      expect(magazineTitles).toEqual(['Magazine A', 'Magazine B', 'Magazine C']);
    });
  ...
  describe('Error Handling Utilities', () => {
    it('creates standardized error responses', () => {
      const createErrorResponse = (error: string, message: string, statusCode: number = 400) => {
        return {
          success: false,
          error,
          message,
      ...
      const validationError = createErrorResponse('Validation failed', 'Invalid input data', 400);
      const serverError = createErrorResponse('Internal server error', 'Something went wrong', 500);
