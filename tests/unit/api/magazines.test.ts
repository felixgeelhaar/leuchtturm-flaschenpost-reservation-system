...
describe('/api/magazines', () => {
  ...
  describe('GET /api/magazines', () => {
    ...
    it('returns empty array when database is not available', async () => {
      mockDatabaseService.getActiveMagazines.mockRejectedValue(new Error('Database connection failed'));
    ...
    it('handles general errors gracefully', async () => {
      mockDatabaseService.logDataProcessing.mockRejectedValue(new Error('Logging failed'));
      ...
      expect(data.message).toBe('Es ist ein Fehler beim Laden der Magazin-Ausgaben aufgetreten.');
    });
    ...
    it('handles magazines with different availability', async () => {
      ...
      mockDatabaseService.getActiveMagazines.mockResolvedValue(magazinesWithZeroAvailable);
