...
describe('/api/magazines', () => {
  ...
  describe('GET /api/magazines', () => {
    ...
    it('handles general errors gracefully', async () => {
      ...
      // When logging fails, it's caught and returns empty array with 200
      expect(response.status).toBe(200);
      expect(data.success).toBe(true);
      expect(data.data).toEqual([]);
      expect(data.count).toBe(0);
    ...
    it('handles logging errors but continues with magazine fetch', async () => {
      ...
      // When logging fails, both logging and getActiveMagazines are in the same try-catch
      // so it returns empty array instead of the mocked magazines
      expect(response.status).toBe(200);
      expect(data.success).toBe(true);
      expect(data.data).toEqual([]);
      expect(data.count).toBe(0);
