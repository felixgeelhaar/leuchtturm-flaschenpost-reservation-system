...
const createValidPickupData = (overrides: Partial<typeof validFormDataPickup> = {}) => {
...
describe('/api/reservations', () => {
  ...
  describe('POST /api/reservations', () => {
    it('creates reservation with pickup delivery method', async () => {
      ...
      if (response.status !== 201) {
        ...
        process.stdout.write(`Response body: ${JSON.stringify(result, null, 2)}\n`);
      }
    ...
    it('validates pickup location for pickup delivery', async () => {
      ...
      expect(response.status).toBe(400);
      expect(result.success).toBe(false);
      expect(result.errors.some((e: any) => e.field === 'pickupLocation')).toBe(true);
    });
    ...
    it('handles database errors gracefully', async () => {
      mockDb.getMagazineById.mockRejectedValue(new Error('Database connection failed'));
    ...
    it('validates magazine ID format', async () => {
      ...
      expect(response.status).toBe(400);
      expect(result.success).toBe(false);
      expect(result.errors.some((e: any) => e.field === 'magazineId')).toBe(true);
    });
    ...
    it('validates pickup date must be in future', async () => {
      ...
      expect(response.status).toBe(400);
      expect(result.success).toBe(false);
      expect(result.errors.some((e: any) => e.field === 'pickupDate')).toBe(true);
    });
    ...
    it('handles database errors in reservation creation gracefully', async () => {
      mockDb.createReservation.mockRejectedValue(new Error('Reservation creation failed'));
    ...
    it('handles consent recording errors gracefully', async () => {
      mockDb.recordConsent.mockRejectedValue(new Error('Consent recording failed'));
    ...
    it('validates deliveryMethod enum', async () => {
      ...
      expect(response.status).toBe(400);
      expect(result.success).toBe(false);
      expect(result.errors.some((e: any) => e.field === 'deliveryMethod')).toBe(true);
    });
    ...
    it('handles different IP addresses for rate limiting', async () => {
      ...
      const rateLimitedRequest = new Request('http://localhost/api/reservations', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'x-forwarded-for': ip1,
      ...
      const rateLimitedResponse = await POST({ request: rateLimitedRequest } as any);
      expect(rateLimitedResponse.status).toBe(429);

      // But request from different IP should work
      const differentIpRequest = new Request('http://localhost/api/reservations', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'x-forwarded-for': ip2,
      ...
      const differentIpResponse = await POST({ request: differentIpRequest } as any);
  ...
  describe('GET /api/reservations', () => {
    it('returns authentication required message', async () => {
      ...
      const response = await GET({ request, url: new URL('http://localhost/api/reservations') } as any);
      const result = await response.json();
